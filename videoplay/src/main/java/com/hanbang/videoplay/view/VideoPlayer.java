 ...
 */
public abstract class VideoPlayer extends FrameLayout implements JCMediaPlayerListener, View.OnClickListener, SeekBar.OnSeekBarChangeListener, View.OnTouchListener, TextureView.SurfaceTextureListener {
    ...
    protected int mSeekTimePosition = 0;
    ...
    public boolean setUp(String url, int screen, Object... objects) {
        if ((System.currentTimeMillis() - CLICK_QUIT_FULLSCREEN_TIME) < FULL_SCREEN_NORMAL_DELAY)
            return false;
        this.currentState = MediaStatus.CURRENT_STATE_NORMAL;
        this.url = url;
        this.objects = objects;
        this.currentScreen = screen;
        setUiWitStateAndScreen(MediaStatus.CURRENT_STATE_NORMAL);
        return true;
    }
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start) {
            ...
            if (currentState == MediaStatus.CURRENT_STATE_NORMAL || currentState == MediaStatus.CURRENT_STATE_ERROR) {
                if (!url.startsWith("file") && !JCUtils.isWifiConnected(getContext()) && !WIFI_TIP_DIALOG_SHOWED) {
                    showWifiDialog();
                    return;
                }
                prepareVideo();
                onEvent(currentState != MediaStatus.CURRENT_STATE_ERROR ? JCBuriedPoint.ON_CLICK_START_ICON : JCBuriedPoint.ON_CLICK_START_ERROR);
            } else if (currentState == MediaStatus.CURRENT_STATE_PLAYING) {
                pause();
            } else if (currentState == MediaStatus.CURRENT_STATE_PAUSE) {
                start();
            } else if (currentState == MediaStatus.CURRENT_STATE_AUTO_COMPLETE) {
                onEvent(JCBuriedPoint.ON_CLICK_START_AUTO_COMPLETE);
                prepareVideo();
            }
        } else if (i == R.id.fullscreen) {
            if (currentState == MediaStatus.CURRENT_STATE_AUTO_COMPLETE) return;
        ...
        } else if (i == R.id.surface_container && currentState == MediaStatus.CURRENT_STATE_ERROR) {
            prepareVideo();
        }
    }

    public void pause() {
        if (currentState == MediaStatus.CURRENT_STATE_PLAYING) {
            AudioManager mAudioManager = (AudioManager) getContext().getSystemService(Context.AUDIO_SERVICE);
            mAudioManager.abandonAudioFocus(onAudioFocusChangeListener);
            onEvent(JCBuriedPoint.ON_CLICK_PAUSE);
            JCMediaManager.instance().mediaPlayer.pause();
            setUiWitStateAndScreen(MediaStatus.CURRENT_STATE_PAUSE);
        }
    }

    public void start() {
        if (currentState == MediaStatus.CURRENT_STATE_PAUSE || currentState == MediaStatus.CURRENT_STATE_PREPAREING) {

            AudioManager mAudioManager = (AudioManager) getContext().getSystemService(Context.AUDIO_SERVICE);
            mAudioManager.requestAudioFocus(onAudioFocusChangeListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
            onEvent(JCBuriedPoint.ON_CLICK_RESUME);
            JCMediaManager.instance().mediaPlayer.start();
            setUiWitStateAndScreen(MediaStatus.CURRENT_STATE_PLAYING);
    ...
    public void prepareVideo() {
        ...
        setUiWitStateAndScreen(MediaStatus.CURRENT_STATE_PREPAREING);
    }

    @Override
    public void onPrepared() {

        if (currentState != MediaStatus.CURRENT_STATE_PREPAREING) return;
        start();
...
//        JCVideoPlayerManager.setLastListener(null);
    ...
    public void setUiWitStateAndScreen(int state) {
        currentState = state;
        switch (currentState) {
            case MediaStatus.CURRENT_STATE_NORMAL:
                if (isCurrentMediaListener()) {
                    cancelProgressTimer();
                    JCMediaManager.instance().releaseMediaPlayer();
                }
                break;
            case MediaStatus.CURRENT_STATE_PREPAREING:
                resetProgressAndTime();
                break;
            case MediaStatus.CURRENT_STATE_PLAYING:
                startProgressTimer();
                break;
            case MediaStatus.CURRENT_STATE_PAUSE:
                startProgressTimer();
                break;
            case MediaStatus.CURRENT_STATE_ERROR:
                if (isCurrentMediaListener()) {
                    JCMediaManager.instance().releaseMediaPlayer();
                }
                break;
            case MediaStatus.CURRENT_STATE_AUTO_COMPLETE:
                cancelProgressTimer();
            ...
            if (currentScreen == SCREEN_WINDOW_FULLSCREEN) {
                ...
                animator.addListener(new Animator.AnimatorListener() {
                    ...
                    public void onAnimationEnd(Animator animator) {
                        ViewGroup vp = (ViewGroup) (JCUtils.scanForActivity(getContext())).findViewById(Window.ID_ANDROID_CONTENT);
                        vp.removeView(VideoPlayer.this);
                    }
    ...
    public void autoFullscreenLeft() {
        if ((System.currentTimeMillis() - lastAutoFullscreenTime) > 2000
                ...
                && currentState == MediaStatus.CURRENT_STATE_PLAYING
                && currentScreen != SCREEN_WINDOW_TINY) {
            ...
            if (currentScreen != SCREEN_WINDOW_FULLSCREEN) {
                ...
                if (getRotation() >= 80) {
                    ObjectAnimator animator = ObjectAnimator.ofFloat(this, "rotation", getRotation(), -90.0f);
                    animator.setDuration(AnimationDuration).start();
                } else if (getRotation() <= -80) {
                    ObjectAnimator animator = ObjectAnimator.ofFloat(this, "rotation", getRotation(), 90.0f);
                    animator.setDuration(AnimationDuration).start();
                }
            }
    ...
    public void onBufferingUpdate(int percent) {
        if (currentState == MediaStatus.CURRENT_STATE_PLAYING_BUFFERING_START) {
            currentState = MediaStatus.CURRENT_STATE_PLAYING;
        }

        if (currentState != MediaStatus.CURRENT_STATE_NORMAL && currentState != MediaStatus.CURRENT_STATE_PREPAREING) {
    ...
    public void onError(int what, int extra) {
        if (what != 38 && what != -38) {
            setUiWitStateAndScreen(MediaStatus.CURRENT_STATE_ERROR);
        }
        AudioManager mAudioManager = (AudioManager) getContext().getSystemService(Context.AUDIO_SERVICE);
        mAudioManager.abandonAudioFocus(onAudioFocusChangeListener);
    }
    ...
    public void onInfo(int what, int extra) {
        if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START) {
            ...
            setUiWitStateAndScreen(MediaStatus.CURRENT_STATE_PLAYING_BUFFERING_START);
...
//        surface.release();
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (currentState != MediaStatus.CURRENT_STATE_PLAYING &&
                currentState != MediaStatus.CURRENT_STATE_PAUSE) return;
    ...
    public void startWindowFullscreen(boolean isRight) {
        ...
        try {
            ...
            ObjectAnimator animator = ObjectAnimator.ofFloat(jcVideoPlayer, "rotation", 0.0f, isRight ? -90.0f : 90.0f);
            animator.setDuration(AnimationDuration).start();
...
//        }
    ...
    public class ProgressTimerTask extends TimerTask {
        @Override
        public void run() {
            if (currentState == MediaStatus.CURRENT_STATE_PLAYING || currentState == MediaStatus.CURRENT_STATE_PAUSE) {
        ...
        public void onAudioFocusChange(int focusChange) {
            ...
            switch (focusChange) {
                ...
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT://用于指示音频重点短暂丧失。
                    pause();
                    break;
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK://用于指示音频重点短暂丧失在音频焦点，如果它想继续打球可以降低其输出音量的失败者（也被称为“回避”），作为新的焦点所有者不需要别人沉默。
                    break;
            }
        }
    };
    ...
    public static class JCAutoFullscreenListener implements SensorEventListener {
        ...
        public void onSensorChanged(SensorEvent event) {//可以得到传感器实时测量出来的变化值
            ...
            if (x < -11) {
                //direction right
                if (JCVideoPlayerManager.listener() != null) {
                    JCVideoPlayerManager.listener().autoFullscreenRight();
                }
            } else if (x > 11) {
                //direction left
                if (JCVideoPlayerManager.listener() != null) {
